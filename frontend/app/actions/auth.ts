...
type AuthAction = "fazer login" | "criar a conta" | "alterar a senha";
...
// NOVA ACTION: Recuperação de Senha
export async function resetPasswordAction(
	...
	try {
		// Certifique-se de que a rota /auth/reset-password existe no seu backend
	...
	// Se o seu backend não retorna um token (apenas sucesso), 
	// você pode precisar de uma lógica diferente de consumeAuthResponse aqui.
	if (!res.ok) {
		const backendMessage = await readErrorMessage(res, "");
		return { error: backendMessage || "Erro ao alterar senha." };
	}

	redirect("/"); // Redireciona para o login após sucesso
}
