...
async function resetPassword(payload: {
  ...
  if (client) {
    const passwordHash = await bcrypt.hash(password, 10);
    await clientModel.updateClientPassword(client.cpf, passwordHash);
    return;
  }
  ...
  if (admin) {
    const passwordHash = await bcrypt.hash(password, 10);
    await adminModel.updateAdminPassword(admin.admin_id, passwordHash);
    return;
  }
...
export { registerClient, login, getAuthenticatedUser, resetPassword };
